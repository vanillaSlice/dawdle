...
class TestContact(TestBase):
    ...
    def test_index_POST_no_email_not_authenticated(self):
        ...
        del data['email']
        self._assert_index_POST_bad_request(data, b'Please enter an email')
    ...
    def test_index_POST_no_subject(self):
        data = self._get_mock_contact_data()
        del data['subject']
        self._assert_index_POST_bad_request(data, b'Please enter a subject')
    ...
    def test_index_POST_subject_greater_than_max(self):
        ...
        self._assert_index_POST_bad_request(
            data,
            b'Subject must be between 1 and 256 characters',
        )

    def test_index_POST_no_message(self):
        data = self._get_mock_contact_data()
        del data['message']
        self._assert_index_POST_bad_request(data, b'Please enter a message')
    ...
    def test_index_POST_message_greater_than_max(self):
        ...
        self._assert_index_POST_bad_request(
            data,
            b'Message must be between 1 and 1000 characters',
        )
    ...
    def _assert_index_POST_response(self, data, status_code, text):
    ...
    def _assert_index_POST_ok(self, data):
        self._assert_index_POST_response(
            data,
            200,
            b'We have received your message',
        )

    def _assert_index_POST_bad_request(self, data, text):
        self._assert_index_POST_response(data, 400, text)

    def _assert_index_POST_internal_server_error(self, data):
        self._assert_index_POST_response(data, 500, b'Could not send message')
